  ...
  <div class="content">
    ...
    <div class="container">
      ...
      <div class="post">
        ...
        <textarea aria-label="titre de la publication" type="title" id="title" name="title" placeholder="Titre"
          v-model="text" maxlength="150" @input="check" />
        <input aria-label="Ajoutez votre image" ref="fileInput" style="display: none" type="file" name="image"
          @change="onFileSelected" accept="image/*" />
        <button id="add-file-button" @click="$refs.fileInput.click()">+ Ajouter une image</button>
        <div class="selected-file-name">{{ selectedFile.name }}</div>
      </div>
      <div id="form-validate-button">
        <button class="form-validate-button" id="form-validate-button" @click="createPost" type="submit"
          :disabled="isDisabled">
          Publier
        </button>
      </div>
